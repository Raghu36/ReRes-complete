...
cnn = unet()
sgd = SGD(lr=0.99, decay=1e-5, momentum=0.5, nesterov=True)
cnn.compile(
...
for x in range(128):
    for y in range(128):
        for z in range(n_train):
            ...
            y_train[z][x][y][1] = 255
'''
gap = numpy.ones((128, 0, N_CHANNEL)) * 63
epoch = 1
...
while True:
    ...
    for sample in range(1, n_train):
        ...
        #org = len(original)
        #krp = original.reshape(org, -1)
        #forg = len(foriginal)
        #fkrp = foriginal.reshape(forg, -1)
        #clf = AdaBoostClassifier(base_estimator = cnn)
        #clf.fit(krp, fkrp)
