...
RESULT = '../../data/offline-data/black-and-white-images/results'
...
for image_id in range(1, n_train + 1):
    x_train[image_id - 1] = misc.imread(LOW + '/' + str(image_id) + '.jpg')
    y_train[image_id - 1] = misc.imread(HIGH + '/' + str(image_id) + '.jpg')
print x_train.shape
...
for x in range(128):
    for y in range(128):
        for z in range(n_train):
            ...
            y_train[z][x][y][1] = 255
'''
gap = numpy.ones((128, 0, 3)) * 255
epoch = 1
while True:
    cnn.fit(x_train, y_train, batch_size=batch_size, verbose=2)
    for sample in range(1, n_train):
        ...
        to_save = numpy.concatenate((gap, original,), axis=1)
        misc.imsave(RESULT + '/' + str(epoch) + '-sample-' + str(sample) + '.jpg', to_save)
    epoch += 1
